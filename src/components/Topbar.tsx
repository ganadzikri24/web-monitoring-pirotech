...
import { Bell, Search, User, Menu, Info, AlertTriangle, CheckCircle, Flame, X } from "lucide-react";
...
export default function Topbar({ onMenuClick = () => {} }) {
  ...
  const [toastNotif, setToastNotif] = useState<NotificationData | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubscribe = listenToNotifications(10, user?.uid, !!isAdmin, (data) => {
      ...
      if (data.length > 0) {
        ...
        // Show red dot if the latest is newer than what was stored
        if (latest.timestamp > storedLastRead) {
          setHasUnread(true);
        }

        // Show toast if it's NEW in this active session
        if (latest.timestamp > lastSeenRef.current) {
  ...
  const handleNotifClick = (notif: NotificationData) => {
    if (notif.title === "Pembakaran Selesai") {
          ...
          >
            <div className="p-4 flex-1">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <p className={`text-sm font-bold ${getNotifColor(toastNotif.type)}`}>{toastNotif.title}</p>
              </div>
              <p className={`text-xs leading-relaxed ml-6 ${toastNotif.type === 'warning' || toastNotif.type === 'critical' ? 'text-foreground font-medium' : 'text-brand-sage'}`}>
                {toastNotif.message}
              </p>
            </div>
            <button 
              onClick={(e) => { e.stopPropagation(); setToastNotif(null); }}
              className="px-3 hover:bg-black/5 transition-colors flex items-center justify-center"
            >
                  ...
                  {notifications.length > 0 ? (
                    notifications.map((notif) => (
                      <div 
                        ...
                        className={`p-3 border-b border-card-border/50 transition-colors ${getNotifStyle(notif.type)} ${notif.title === "Pembakaran Selesai" ? 'cursor-pointer hover:opacity-80' : ''}`}
                      >
